...
namespace SumoX.Report.Engine.Definition
{
	...
	internal abstract class ChartBase : IDisposable
	{
        ...
        internal ChartBase(Runtime.Report r, Row row, Chart c, MatrixCellEntry[,] m, Expression showTooltips, Expression showTooltipsX,Expression _ToolTipYFormat, Expression _ToolTipXFormat)
		{
			_ChartDefn = c;
			_row = row;
			_DataDefn = m;
		...
		internal virtual void Draw(Runtime.Report rpt)
		{
		}
		...
		internal void Save(Runtime.Report rpt, System.IO.Stream stream, ImageFormat im)
		{
			if (_bm == null)
				Draw(rpt);
            _mf.Save(stream, im);
	//		_bm.Save(stream, im);
		...
		internal System.Drawing.Imaging.Metafile Image(Runtime.Report rpt) // GJL 14082008 Using Vector Graphics
		{
			if (_bm == null)
				Draw(rpt);
		...
		protected Brush[] SeriesBrush(Runtime.Report rpt, Row row, ReportDefn defn)
		{
		...
		protected void DrawChartStyle(Runtime.Report rpt, Graphics g)
		{
		...
		protected System.Drawing.Rectangle DrawLegend(Runtime.Report rpt, Graphics g, bool bMarker, bool bBeforePlotDrawn)
		{
		...
		protected Size[] DrawLegendMeasure(Runtime.Report rpt, Graphics g, Font f, StringFormat sf, SizeF maxSize, out int maxWidth, out int maxHeight)
		{
		...
		protected void DrawPlotAreaStyle(Runtime.Report rpt, Graphics g, System.Drawing.Rectangle crect)
		{
	...
	//				g.Clip = saver;
		...
		protected void DrawTitle(Runtime.Report rpt, Graphics g, Title t, System.Drawing.Rectangle rect)
		{
			if (t == null)
				return;
		...
		protected Size DrawTitleMeasure(Runtime.Report rpt, Graphics g, Title t)
		{
        ...
        protected void DrawCategoryLabel(Runtime.Report rpt, Graphics g, string t, Style a, System.Drawing.Rectangle rect)
        {
        ...
        protected Size DrawCategoryTitleMeasure(Runtime.Report rpt, Graphics g, string t, Style a)
        {
            Size size = Size.Empty;
            Row r = FirstChartRow(rpt);
		...
		protected object GetCategoryValue(Runtime.Report rpt, int row, out TypeCode tc)
		{
		...
		protected double GetDataValue(Runtime.Report rpt, int row, int col)
		{
		 ...
		 * 05122007AJM */
        protected string GetPlotType(Runtime.Report rpt, int row, int col)
        {
            try
            {
...
// 20022008 AJM GJL
        protected string GetYAxis(Runtime.Report rpt, int row, int col)
        {
            try
            {
        ...
        protected bool getNoMarkerVal(Runtime.Report rpt, int row, int col)
        {
            try
            {
        ...
        protected String getLineSize(Runtime.Report rpt, int row, int col)
        {
            try
            {
        ...
        protected String getColour(Runtime.Report rpt, int row, int col)
        {
            try
            {
        ...
        protected double GetDataValue(Runtime.Report rpt, int row, int col, int xyb)
        {
        ...
        protected string GetDataValueString(Runtime.Report rpt, int row, int col)
        {
            MatrixCellEntry mce = _DataDefn[row, col];
            if (mce == null)
        ...
        protected Brush GetSeriesBrush(Runtime.Report rpt, int row, int col)
        {
		...
		protected void DrawDataPoint(Runtime.Report rpt, Graphics g, Point p, int row, int col)
		{
			DrawDataPoint(rpt, g, p, System.Drawing.Rectangle.Empty, row, col);
		}

		protected void DrawDataPoint(Runtime.Report rpt, Graphics g, System.Drawing.Rectangle rect, int row, int col)
		{
			DrawDataPoint(rpt, g, Point.Empty, rect, row, col);
		}

		void DrawDataPoint(Runtime.Report rpt, Graphics g, Point p, System.Drawing.Rectangle rect, int row, int col)
		{
			MatrixCellEntry mce = _DataDefn[row, col];
			if (mce == null)
				return;					// Not sure what this really means TODO:
		...
		protected string GetSeriesValue(Runtime.Report rpt, int iCol)
		{
        ...
        protected bool ShowRightYAxis(Runtime.Report rpt)
        {
		...
		protected void GetMaxMinDataValue(Runtime.Report rpt, out double max, out double min, int xyb,int WhichYAxis)
		{
		...
		void GetMaxMinDataValueStacked(Runtime.Report rpt, out double max, out double min)
		{
			min = double.MaxValue;
			max = double.MinValue;
		...
		protected Brush[] GetSeriesBrushes(Runtime.Report rpt, Row row, ReportDefn defn)
		{
...
// 20022008 AJM GJL
        ...
        protected void GetValueMaxMin(Runtime.Report rpt, ref double max, ref double min, int xyb, int WhichYAxis)// 20022008 AJM GJL
		{
...
// 20022008 AJM GJL
		...
		protected void AdjustMargins(System.Drawing.Rectangle legendRect, Runtime.Report rpt, Graphics g)
		{
        ...
        protected void SetIncrementAndInterval(Runtime.Report rpt, Axis a, double min, double max, out double incr, out int interval)
        {
		...
		private Row FirstChartRow(Runtime.Report rpt)
		{
