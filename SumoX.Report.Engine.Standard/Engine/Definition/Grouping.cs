...
namespace SumoX.Report.Engine.Definition
{
	...
	internal class Grouping : ReportLink
	{
		...
		internal void ResetHideDuplicates(Runtime.Report rpt)
		{
			if (_HideDuplicates == null)
				return;
        ...
        internal bool PageBreakCondition(Runtime.Report r, Row row, bool SeMancaDefinizione)
        {
            bool result;
            if (_PageBreakCondition != null)
		...
		internal int GetIndex(Runtime.Report rpt)
		{
			WorkClass wc = GetValue(rpt);
			return wc.index;
		}

		internal void SetIndex(Runtime.Report rpt, int i)
		{
			WorkClass wc = GetValue(rpt);
			wc.index = i;
			return;
		...
		internal Rows GetRows(Runtime.Report rpt)
		{
			WorkClass wc = GetValue(rpt);
			return wc.rows;
		}

		internal void SetRows(Runtime.Report rpt, Rows rows)
		{
			WorkClass wc = GetValue(rpt);
			wc.rows = rows;
			return;
