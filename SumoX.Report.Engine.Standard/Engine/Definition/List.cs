...
namespace SumoX.Report.Engine.Definition
{
	...
	internal class List : DataRegion
	{
			...
			foreach(XmlNode xNodeLoop in xNode.ChildNodes)
			{
				...
				switch (xNodeLoop.Name)
				{
					...
					case "DataInstanceElementOutput":
						_DataInstanceElementOutput = Definition.DataInstanceElementOutput.GetStyle(xNodeLoop.InnerText, OwnerReport.rl);
						break;
					default:	
		...
		private void RunSetGrouping(Runtime.Report rpt, WorkClass wc)
		{
		...
		private void PrepGroups(Runtime.Report rpt, WorkClass wc)
		{
			if (_Grouping == null)
				return;
...
//			p.YOffset += (Top == null? 0: this.Top.Points);
        ...
        internal override void RemoveWC(Runtime.Report rpt)
        {
            base.RemoveWC(rpt);
		...
		private void RunGroupsSetGroups(Runtime.Report rpt, WorkClass wc, List<GroupEntry> groupEntries)
		{
			// set the group entry value
		...
		internal float HeightOfList(Runtime.Report rpt, Graphics g, Row r)
		{		   
