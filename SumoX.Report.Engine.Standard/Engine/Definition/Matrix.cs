...
namespace SumoX.Report.Engine.Definition
{
	...
	internal class Matrix : DataRegion
	{
		...
		internal void RunReset(Runtime.Report rpt)
		{
			RemoveValue(rpt);
		}
		...
		internal MatrixCellEntry[,] RunBuild(Runtime.Report rpt, out int numRows, out int numCols)
		{
			WorkClass wc = GetValue(rpt);
			Rows _Data = wc.Data;
		...
		void HandleRowGrouping(Runtime.Report rpt, WorkClass wc, Rows rows, Row r, MatrixEntry m, int rgi, int iRow, ref int maxRows)
		{
		...
		void HandleColumnGrouping(Runtime.Report rpt, WorkClass wc, Rows rows, Row r, MatrixEntry m, int cgi, int iRow, ref int maxColumns)
		{
		...
		int RunCountSubtotalColumns(Runtime.Report rpt, WorkClass wc, MatrixEntry m, int level)
		{
		...
		int RunCountSubtotalRows(Runtime.Report rpt, WorkClass wc, MatrixEntry m, int level)
		{
			// Get the number of static columns
		...
		void RunColumnHeaders(Runtime.Report rpt, WorkClass wc, MatrixEntry m, MatrixCellEntry[,] matrix, Rows _Data, int iRow, ref int iColumn, int level)
		{
			foreach (MatrixEntry ame in m.GetSortedData(rpt))
			{
		...
		void RunColumnStaticHeaders(Runtime.Report rpt, WorkClass wc, MatrixCellEntry[,] matrix, Rows _Data, int iRow, int iColumn, int level)
		{
		...
		MatrixCellEntry RunGetColumnHeader(Runtime.Report rpt, MatrixEntry me, Rows _Data)
		{
			ReportItem ri;
		...
		void RunDataColumn(Runtime.Report rpt, WorkClass wc, MatrixEntry rm, MatrixEntry cm, MatrixCellEntry[,] matrix, Rows _Data, int iRow, ref int iColumn, int level, int rowcell)
		{
			BitArray andData;
		...
		void RunDataRow(Runtime.Report rpt, WorkClass wc, MatrixEntry rm, MatrixEntry cm, MatrixCellEntry[,] matrix, Rows _Data, ref int iRow, int iColumn, int level)
		{
			int saveColumn;
			int headerRows = _ColumnGroupings.Items.Count;	// number of column headers we have
		...
		void RunRowHeaders(Runtime.Report rpt, WorkClass wc, MatrixEntry m, MatrixCellEntry[,] matrix, Rows _Data, ref int iRow, int iColumn, int level)
		{
			foreach (MatrixEntry ame in m.GetSortedData(rpt))
			{
		...
		void RunRowStaticHeaders(Runtime.Report rpt, WorkClass wc, MatrixCellEntry[,] matrix, Rows _Data, int iRow, int level)
		{
			RowGrouping rg=null;
			int i;
			int iColumn=0;
		...
		MatrixCellEntry RunGetRowHeader(Runtime.Report rpt, MatrixEntry me, Rows _Data)
		{
			ReportItem ri;
		...
		MatrixCellEntry RunGetMatrixCell(Runtime.Report rpt, MatrixEntry me, int iRow, Rows _Data, BitArray rows, int firstRow, int lastRow)
		{
			int headerRows = _ColumnGroupings.Items.Count;	// number of column headers we have
			int rgsCount = this.RowGroupings.StaticCount;	// count of static row groups
			int rowcell = rgsCount == 0? 0: (iRow - headerRows) % rgsCount;
		...
		MatrixCellEntry RunGetMatrixCell(Runtime.Report rpt, MatrixEntry me, int rcell, int ccell, Rows _Data, BitArray rows, int firstRow, int lastRow)
		{
		...
		internal Rows GetMyData(Runtime.Report rpt)
		{
			WorkClass wc = GetValue(rpt);
			return wc.Data;
		}

		internal void SetMyData(Runtime.Report rpt, Rows data)
		{
			WorkClass wc = GetValue(rpt);
			wc.Data = data;
		}
		...
		private void SetGroupingValues(Runtime.Report rpt, MatrixCellEntry mce)
		{
			WorkClass wc = GetValue(rpt);
		...
		private void SetGroupingValuesInit(Runtime.Report rpt, Rows data, MatrixEntry rme, MatrixEntry cme)
		{
		...
		private void SetGroupingValuesMe(Runtime.Report rpt, Rows data, MatrixEntry me)
		{
			if (me == null)
				return;
			// handle the column grouping
		...
		private void SetGrouping(Runtime.Report rpt, Grouping g, MatrixEntry me, Rows data)
		{
			if (g == null)
				return;
		...
		private WorkClass GetValue(Runtime.Report rpt)
		{
		...
		private void RemoveValue(Runtime.Report rpt)
		{
