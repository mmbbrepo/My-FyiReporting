...
using SumoX.Report.Engine.Runtime.PageItems;
...
namespace SumoX.Report.Engine.Definition
{
	...
	internal class ReportItem : ReportLink, IComparable
	{
		...
		internal bool ReportItemElement(XmlNode xNodeLoop)
		{
			switch (xNodeLoop.Name)
			{
				...
				case "DataElementOutput":
					_DataElementOutput = Definition.DataElementOutput.GetStyle(xNodeLoop.InnerText, OwnerReport.rl);
					break;
                case "rd:DefaultName":
		...
		internal bool IsTableOrMatrixCell(Runtime.Report rpt)
		{
		...
		internal float LeftCalc(Runtime.Report rpt)
		{
			WorkClass wc = GetWC(rpt);
		...
		internal float GetOffsetCalc(Runtime.Report rpt)
		{
			WorkClass wc = GetWC(rpt);
			float x;
			if (this._TC != null)		
		...
		internal bool IsHidden(Runtime.Report rpt, Row r)
		{
		...
		internal void SetPageLeft(Runtime.Report rpt)
		{
			if (this._TC != null)		
			{	// must be part of a table
				Table t = _TC.OwnerTable;
		...
		internal void SetPagePositionAndStyle(Runtime.Report rpt, PageItem pi, Row row)
		{
			WorkClass wc = GetWC(rpt);
		...
		internal MatrixCellEntry GetMC(Runtime.Report rpt)
		{
			WorkClass wc = GetWC(rpt);
			return wc.MC;
		}

		internal void SetMC(Runtime.Report rpt, MatrixCellEntry mce)
		{
			WorkClass wc = GetWC(rpt);
			wc.MC = mce;
		}
		...
		internal float WidthOrOwnerWidth(Runtime.Report rpt)
		{
			if (_Width != null)
		...
		internal int WidthCalc(Runtime.Report rpt, Graphics g)
		{
			WorkClass wc = GetWC(rpt);
			int width;
			if (this._TC != null)
		...
		/// above it vertically.
		/// </summary>
		internal float Gap(Runtime.Report rpt)
		{
		...
		/// above it vertically.
		/// </summary>
		internal float RelativeY(Runtime.Report rpt)
		{
			float top = _Top == null? 0: _Top.Points;
			ReportItem saveri=GetReportItemAbove(rpt);
			if (saveri == null)
		...
		private ReportItem GetReportItemAbove(Runtime.Report rpt)
		{
			if (this._YParents == null)
				return null;
		...
		internal string ToolTipValue(Runtime.Report rpt, Row r)
		{
			if (_ToolTip == null)
				return null;
		...
		internal string BookmarkValue(Runtime.Report rpt, Row r)
		{
			if (_Bookmark == null)
				return null;
		...
		internal virtual void RemoveWC(Runtime.Report rpt)
		{
