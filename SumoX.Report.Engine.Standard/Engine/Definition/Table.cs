...
using SumoX.Report.Engine.Functions;
...
namespace SumoX.Report.Engine.Definition
{
	...
	internal class Table : DataRegion
	{
			...
			foreach(XmlNode xNodeLoop in xNode.ChildNodes)
			{
				...
				switch (xNodeLoop.Name)
				{
					...
					case "DetailDataElementOutput":
						_DetailDataElementOutput = Definition.DataElementOutput.GetStyle(xNodeLoop.InnerText, OwnerReport.rl);
						break;
					default:	
		...
		void RunPrep(Runtime.Report rpt, Row row, TableWorkClass wc)
		{
		...
		private void SortGroups(Runtime.Report rpt, List<GroupEntry> groupEntries, TableWorkClass wc)
		{
		...
		private void PrepGroups(Runtime.Report rpt, TableWorkClass wc)
		{
		...
		private void PrepRecursiveGroup(Runtime.Report rpt, TableWorkClass wc)
		{
        ...
        internal void RunGroupsSetGroups(Runtime.Report rpt, TableWorkClass wc, List<GroupEntry> groupEntries)
		{
			// set the group entry value
		...
		internal int GetGroupNestCount(Runtime.Report rpt)
		{
			TableWorkClass wc = GetValue(rpt);
			return wc.GroupNestCount;
		}

		internal int WidthInPixels(Runtime.Report rpt, Row row)
		{
		...
		private TableWorkClass GetValue(Runtime.Report rpt)
		{
	...
	class RecursiveCompare: IComparer<Row>
	{
        ...
        internal RecursiveCompare(Runtime.Report r, IExpr pExpr, TypeCode tc, IExpr gExpr)
		{
            rpt = r;
			_Type = tc;
			parentExpr = pExpr;
	...
	class GroupEntryCompare: IComparer<GroupEntry>
	{
		...
		internal GroupEntryCompare(Runtime.Report r, TableWorkClass w )
		{
			rpt = r;
			wc = w;
		}
