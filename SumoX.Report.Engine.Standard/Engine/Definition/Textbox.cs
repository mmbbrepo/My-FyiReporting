...
namespace SumoX.Report.Engine.Definition
{
	...
	internal class Textbox : ReportItem
	{
		...
		Runtime.Report lastValueForReport = null;
			...
			foreach(XmlNode xNodeLoop in xNode.ChildNodes)
			{
				...
				switch (xNodeLoop.Name)
				{
					...
					case "DataElementStyle":
						_DataElementStyle = Definition.DataElementStyle.GetStyle(xNodeLoop.InnerText, OwnerReport.rl);
						break;
					default:
		...
		internal void RecordPageReference(Runtime.Report rpt, Page p, Row r)
		{
			if (_ExprReferences == null)
				return;
		...
		internal void ResetPrevious(Runtime.Report rpt)
		{
		...
		internal string RunText(Runtime.Report rpt, Row row)
		{
		...
		internal float RunTextCalcHeight(Runtime.Report rpt, Graphics g, Row row)
		{
			return RunTextCalcHeight(rpt, g, row, null);
		}
		
		internal float RunTextCalcHeight(Runtime.Report rpt, Graphics g, Row row, PageTextHtml pth)
		...
		internal object Evaluate(Runtime.Report rpt, Row r)
		{
		...
		internal bool IsHtml(Runtime.Report rpt, Row row)
		{
		...
		internal int RunCount(Runtime.Report rpt)
		{
			TextboxRuntime tbr = TextboxRuntime.GetTextboxRuntime(rpt, this);
	...
	class TextboxRuntime
	{
		...
		static internal TextboxRuntime GetTextboxRuntime(Runtime.Report rpt, Textbox tb)
		{
