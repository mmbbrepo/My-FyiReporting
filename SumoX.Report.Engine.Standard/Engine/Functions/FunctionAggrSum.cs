...
namespace SumoX.Report.Engine.Functions
{
	...
	internal class FunctionAggrSum : FunctionAggr, IExpr, ICacheData
	{
		...
		public object EvaluateObject(Runtime.Report rpt, Row row)
		{
			bool bSave;
		...
		private ODecimal GetValueDecimal(Runtime.Report rpt)
		{
			return rpt.Cache.Get(_key) as ODecimal;
		}

		private ODouble GetValueDouble(Runtime.Report rpt)
		{
			return rpt.Cache.Get(_key) as ODouble;
		}

		private object GetValueObject(Runtime.Report rpt)
		{
			return rpt.Cache.Get(_key);
		}

		private void SetValue(Runtime.Report rpt, double d)
		{
			rpt.Cache.AddReplace(_key, new ODouble(d));
		}

		private void SetValue(Runtime.Report rpt, decimal d)
		{
			rpt.Cache.AddReplace(_key, new ODecimal(d));
		}

		private void SetValue(Runtime.Report rpt, object d)
		{
