...
namespace SumoX.Report.Engine.Render.ExcelConverter
{
	...
	{
		Graphics g;
		public Runtime.Report Report { get; set; }
		...
		public void AddImage(Image image, int pictureIndex)
		{
		...
		public void AddLine(Line line, float borderWidth)
		{
		...
		private void FillAbsolutePosition(ReportItem reportItem, ref float top, ref float left)
		{
			for(ReportLink rl = reportItem.Parent; rl != null; rl = rl.Parent) {
				...
				if((rl is DataRegion || rl is Rectangle) && !(rl is CustomReportItem)) {
