...
using SumoX.Report.Engine.Render.ExcelConverter;
...
namespace SumoX.Report.Engine.Render
{
	...
	internal class RenderExcel2007 : IPresent
	{
		...
		public RenderExcel2007(Runtime.Report rep, IStreamGen sg)
		{
			report = rep;
				...
				for(int j = 0; j < builderRow.Cells.Count; j++) {
					...
					if(builderCell.ReportItem != null) {
						...
						if(rightAttach > 0 || bottomAttach > 0) {
                            ...
                            RegionUtil.SetBorderTop((int)xssfStyle.BorderTop, new CellRangeAddress(rowIndex, rowIndex, colIndex, colIndex), worksheet);
